...
## 14. Verification — March 15, 2026
✅ Confirmed that the following files contain the expected changes (as requested):

- **`supabase/functions/aisbs-chat/index.ts`**
  - `max_tokens` is set to `32768` across all provider requests.
  - The `executionSystemPrompt` still contains the mandatory **COMPLETION PRIORITY** instruction.

- **`src/pages/PromptExecutionPage.tsx`**
  - `handleDownloadPDF` now clones the rendered DOM into an off-screen wrapper before invoking `html2pdf`.
  - The live output element uses `id="execution-output"` for accurate PDF capture.

- **`src/pages/ProblemPage.tsx`**
  - Same PDF export logic and `execution-output` targeting applied here as well.

All changes were verified by reading the current file contents directly in the workspace. No further edits were required.