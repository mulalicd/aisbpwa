...
export default function ProblemPage() {
  ...
  // Execution console state (same as PromptExecutionPage)
  ...
  const handleDownloadPDF = async () => {
    ...
    wrapper.innerHTML =
      ...
      `<div style="font-size:20px;font-weight:800;color:#1e293b;">${title}</div>` +
    ...
    const opt = {
      margin: [10, 10, 10, 10],
      filename: `AISBS-Report-${new Date().toISOString().slice(0, 10)}.pdf`,
    ...
    <AppLayout>
      <div className="w-full py-6 sm:py-8 px-4 sm:px-6 md:px-10 lg:px-14 xl:px-20">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          ...
          {/* Prompt — with full Execution Console */}
          {problem.prompt && (
            <motion.div className="mb-10" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <Collapsible open={promptExpanded} onOpenChange={setPromptExpanded}>
                <div className="rounded-xl border border-primary/20 bg-primary/5 overflow-hidden">
                  <pre className="p-4 sm:p-5 text-xs sm:text-sm text-foreground leading-relaxed whitespace-pre-wrap font-mono overflow-x-auto max-h-48">
                    {problem.prompt.substring(0, 600)}...
                  </pre>
                  <CollapsibleContent>
                  ...
                  <CollapsibleTrigger asChild>
                    <button className="w-full py-2 text-xs font-medium text-primary hover:bg-primary/10 transition-colors border-t border-primary/10 flex items-center justify-center gap-1">
                      {promptExpanded ? <>Collapse prompt <ChevronUp className="h-3 w-3" /></> : <>View full prompt ({(problem.prompt.length / 1000).toFixed(1)}K chars) <ChevronDown className="h-3 w-3" /></>}
                    </button>
                  </CollapsibleTrigger>
                </div>
              </Collapsible>

              {/* Full Execution Console (same as PromptExecutionPage) */}
              <AnimatePresence>
                {showPromptExec && (
